...
from ..core.emailcollector import EmailCollector, EXCLUDE_EMAIL
...
class Select(ExceptionHandler):
    ...
    def file_open(self):
        ...
        config_file = tkinter.filedialog.askopenfilename(
            parent=self.get_toplevel(),
            title='Open Email Selection Rules',
            defaultextension='.conf',
            filetypes=(
                ('Email Selection Rules', '*.conf'),
                ),
    ...
    def file_save(self):
        """Save the open email selection rules file."""
        if self._configuration is None:
            tkinter.messagebox.showinfo(
                parent=self.get_toplevel(),
                title='Save',
        ...
        if tkinter.messagebox.askquestion(
            ...
            title='Save',
            message=''.join(
                ('Confirm save email selection rules to\n',
                 self._configuration,
                 )),
    ...
    def file_save_copy_as(self):
        ...
        config_file = tkinter.filedialog.asksaveasfilename(
            parent=self.get_toplevel(),
            title='Save Email selection rules As',
            defaultextension='.conf',
            filetypes=(
                ('Email Selection Rules', '*.conf'),
                ),
    ...
    def clear_selection(self):
        ...
        if tkinter.messagebox.askquestion(
            ...
            message='Confirm request to clear the lists of selected emails.',
            ) != tkinter.messagebox.YES:
            return
        self._clear_email_tags()
        self.emailtextctrl.delete('1.0', tkinter.END)
        ...
        if tkinter.messagebox.askquestion(
            ...
            message=''.join(
                ('Confirm request to cancel exclusion of \n\n',
                 ...
                 'directory in this action; use "Apply" later to do this.',
                 )),
            ) != tkinter.messagebox.YES:
            return
                ...
                if filename in self._email_collector.excluded_emails:
                    tkinter.messagebox.showinfo(
                        ...
                        message=''.join(
                            (filename,
                             '\n\n',
                             'is already one of the emails excluded from the ',
                             'selection.',
                             )))
                    return
                fp = os.path.join(
                    ...
                    if tkinter.messagebox.askquestion(
                        ...
                        message=''.join(
                            (filename,
                             ...
                             '\n\nto exclude email list in selection rules.',                             
                             )),
                        ) != tkinter.messagebox.YES:
                        return
